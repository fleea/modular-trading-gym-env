    ...
    def _calculate_trend(self, env: TrendEnvironment) -> List[float]:
        ...
        for offset in self.trend_offsets:
            if env.current_index - offset >= env.start_index:
                ...
                close_to_close = ((current_data.close - previous_data.close) / previous_data.close)
                close_to_high = ((current_data.close - previous_data.high) / previous_data.high)
                close_to_low = ((current_data.close - previous_data.low) / previous_data.low)
